...
class ProductSearchEngine:
    ...
    def search_products(self, search_query: str = "", suppliers: List[str] = None,
        ...
        if min_height is not None:
            base_query += " AND CAST(HL AS REAL) >= ?"
...
def main():
    ...
    # 1. 关键词搜索框
    st.sidebar.subheader("1. 关键词搜索")
    search_query = st.sidebar.text_input(
        "输入搜索关键词",
        placeholder="SKU、Code、Description或SubCategory的一部分...",
        help="支持操作符: + (AND), - (NOT), or (OR)。例如: rose +red, flower -white, big or large"
    )

    # 2. 供应商选择
    st.sidebar.subheader("2. 供应商筛选")
    ...
    # 3. 高度/长度筛选
    st.sidebar.subheader("3. 高度/长度筛选")
    col_h1, col_h2 = st.sidebar.columns(2)
    with col_h1:
        min_height = st.number_input("最小值", value=None, placeholder="最小", key="min_height")
    with col_h2:
        max_height = st.number_input("最大值", value=None, placeholder="最大", key="max_height")

    # 4. 价格筛选
    st.sidebar.subheader("4. 价格筛选")
    col_p1, col_p2 = st.sidebar.columns(2)
    with col_p1:
        min_price = st.number_input("最低价", value=None, placeholder="最低", key="min_price")
    with col_p2:
        max_price = st.number_input("最高价", value=None, placeholder="最高", key="max_price")

    # 5. 类别筛选
    st.sidebar.subheader("5. 类别筛选")
    ...
    # 搜索按钮
    search_button = st.sidebar.button("🔍 执行搜索", type="primary")

    # 键盘快捷键提示
    st.sidebar.info("💡 提示: 按 Ctrl+Enter 也可执行搜索")

    # 检查是否需要执行搜索（按钮点击或Ctrl+Enter）
    execute_search = search_button

    # 主内容区域
    if execute_search or st.session_state.get('auto_search', False):
        # 重置页码
        if execute_search:
            st.session_state.search_page = 1
        ...
        if not df.empty:
            ...
            if '标价' in df_display.columns:
                df_display['标价'] = df_display['标价'].apply(lambda x: f"${x:.2f}" if pd.notna(x) else "")
            ...
            if total_count > 10:
                ...
                with col_prev:
                    if st.button("⬅️ 上一页", disabled=st.session_state.search_page <= 1):
                        st.session_state.search_page -= 1
                ...
                with col_next:
                    if st.button("下一页 ➡️", disabled=st.session_state.search_page >= total_pages):
                        st.session_state.search_page += 1
                        st.session_state.auto_search = True
                        st.experimental_rerun()
        ...
        # 显示搜索提示
        st.info("👈 请在左侧设置搜索条件，然后点击"执行搜索"按钮")

# 键盘快捷键处理
def handle_keyboard_shortcuts():
    """处理键盘快捷键"""
    # 这里可以添加JavaScript来处理Ctrl+Enter
    # Streamlit的键盘快捷键支持有限，这是一个已知限制
    pass
